...
echo "=== Qwen3.8-Flash-Next single-Spark setup ==="
...
ok "checkpoint: 206/206 safetensors, ${GB} GB"
...
[ "$CAP" = "121" ] || echo "  NOTE: compute capability sm_$CAP (expected sm_121 / GB10). Untested here."
ok "docker + GPU reachable (sm_$CAP)"
...
docker image inspect "$IMAGE" >/dev/null 2>&1 || {
  echo "  pulling $IMAGE (~14 GB) ..."; docker pull "$IMAGE" || die "image pull failed"; }
...
ok "flashnext-one-spark patches present (4)"
...
ok "FP8 dense cast patched in"

# ---- 5. HashK R=4 artifact ------------------------------------------------
if [ ! -f "$REPO/ple_hashk_R4.pt" ]; then
  echo "  building HashK R=4 artifact (~7 min GPU, streams the 51 GB table) ..."
...
ok "HashK R=4 artifact: $(du -h "$REPO/ple_hashk_R4.pt" | cut -f1)"
